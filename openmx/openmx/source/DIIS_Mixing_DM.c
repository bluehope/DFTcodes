...
void DIIS_Mixing_DM(int SCF_iter, double ******ResidualDM, double ******iResidualDM)
{
    ...
    if (g_exx_switch) {
        ...
        ep_atom1 = EXX_Array_EP_Atom1(g_exx);
        ep_atom2 = EXX_Array_EP_Atom2(g_exx);
    }
#endif
    /*---------- until here */
        ...
        if (g_exx_switch) {
            for (spin=0; spin<=SpinP_num; spin++) {
                for (iep=0; iep<nep; iep++) {
                    ...
                    for (i=0; i<TNO1; i++) {
                        for (j=0; j<TNO2; j++) {
                            g_exx_DM[2][spin][iep][i][j] = g_exx_DM[1][spin][iep][i][j];
                        }
                    }
                }
            }
        ...
        if (g_exx_switch) {
            for (spin=0; spin<=SpinP_num; spin++) {
                for (iep=0; iep<nep; iep++) {
                    Gc_AN = ep_atom1[iep]+1;
                    Gh_AN = ep_atom2[iep]+1;
                    TNO1 = Spe_Total_CNO[WhatSpecies[Gc_AN]];
                    TNO2 = Spe_Total_CNO[WhatSpecies[Gh_AN]];
                    for (i=0; i<TNO1; i++) {
                        for (j=0; j<TNO2; j++) {
                            /* Pulay mixing */
                            sum_r = 0.0;
                            sum_i = 0.0;
                            for (pSCF_iter=1; pSCF_iter<=NumMix; pSCF_iter++) {
                                sum_r += alden[pSCF_iter]
                                         *g_exx_DM[pSCF_iter][spin][iep][i][j].r;
                                sum_i += alden[pSCF_iter]
                                         *g_exx_DM[pSCF_iter][spin][iep][i][j].i;
                            }
                            g_exx_DM[0][spin][iep][i][j].r = sum_r;
                            g_exx_DM[0][spin][iep][i][j].i = sum_i;
                        }
                    }
                }
            }
            ...
            for (pSCF_iter=NumSlide; 0<pSCF_iter; pSCF_iter--) {
                for (spin=0; spin<=SpinP_num; spin++) {
                    for (iep=0; iep<nep; iep++) {
                        ...
                        for (i=0; i<TNO1; i++) {
                            for (j=0; j<TNO2; j++) {
                                g_exx_DM[pSCF_iter][spin][iep][i][j]
                                    = g_exx_DM[pSCF_iter-1][spin][iep][i][j];
                            }
                        }
                    }
                }
