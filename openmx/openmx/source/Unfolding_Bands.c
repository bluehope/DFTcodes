    ...
    double ****CntOLP)
{
    ...
    ****************************************************/
    ...
    buildMapRlist();
    if (exitcode==1) {
        for (i=0; i<3; i++) free(unfold_abc[i]);
        free(unfold_abc);
        free(unfold_origin);
        free(unfold_mapN2n);
        for (i=0; i<unfold_Nkpoint+1; i++) free(unfold_kpoint[i]);
        free(unfold_kpoint);
        return;
    }
    ...
    ****************************************************/
    ...
    for (i=0; i<4; i++) free(kj_v[i]);
    free(kj_v);
    for (i=0; i<atomnum; i++) free(weight[i]);
    free(weight);
    ...
    for (i=0; i<atomnum; i++) free(tmpelem[i]);
    free(tmpelem);
    for (i=0; i<3; i++) free(fracabc[i]);
    free(fracabc);
    free(Norbperatom);
    for (i=0; i<unfold_Nkpoint+1; i++) free(unfold_kpoint[i]);
    free(unfold_kpoint);
    ...
    double ****CntOLP)
{
    ...
    ****************************************************/
    ...
    for (i=0; i<2; i++) free(kj_v[i]);
    free(kj_v);
    for (i=0; i<2; i++) for (j=0; j<atomnum; j++) free(kj_v1[i][j]);
    for (i=0; i<2; i++) free(kj_v1[i]);
    free(kj_v1);
    for (i=0; i<atomnum; i++) free(weight[i]);
    free(weight);
    for (i=0; i<atomnum; i++) free(weight1[i]);
    free(weight1);
    for (i=0; i<NR; i++) free(Rlist[i]);
    free(Rlist);
    for (i=0; i<nr; i++) free(rlist[i]);
    ...
    for (i=0; i<atomnum; i++) free(tmpelem1[i]);
    free(tmpelem1);
    for (i=0; i<3; i++) free(fracabc[i]);
    free(fracabc);
    free(Norbperatom);
...
int chkr(int a, int b, int c) {
    int i;
    for (i=0; i<nr; i++) if ((rlist[i][0]==a)&&(rlist[i][1]==b)&&(rlist[i][2]==c)) return i;
    return -99999;
}

int addr(int a, int b, int c) {
    int i, j, ck;
    ck=chkr(a,b,c);
...
void buildrnmap(const int* mapN2n) {
    int i, j, k;
    natom=-999;
    for (i=0; i<atomnum; i++) if (mapN2n[i]>natom) natom=mapN2n[i];
    natom++;
    ...
    for (iR=0; iR<NR; iR++) for (iN=0; iN<atomnum; iN++) {
            int tmpi;
            tmpi=addr(tabr4RN[iR][iN][0],tabr4RN[iR][iN][1],tabr4RN[iR][iN][2]);
            rnmap[tmpi][mapN2n[iN]][0]=iR;
            rnmap[tmpi][mapN2n[iN]][1]=iN;
        }
}

static double dis(const double* a) {
    return sqrt(a[2]*a[2]+a[1]*a[1]+a[0]*a[0]);
}

static double distwovec(const double* a, const double* b) {
    return sqrt((a[2]-b[2])*(a[2]-b[2])+(a[1]-b[1])*(a[1]-b[1])+(a[0]-b[0])*(a[0]-b[0]));
}
...
static int MapR(const int i,const int j,const int k) {
    return _MapR[i+10][j+10][k+10];
}

int AddR(const int a,const int b,const int c) {
    if (MapR(a,b,c)==-1) {
        int i, j;
        int** tmpRlist;
        tmpRlist=(int**)malloc(sizeof(int*)*NR);
        ...
        for (tmpi=0; tmpi<8; tmpi++) {
            ...
            for (tmpj=0; tmpj<8; tmpj++) {
                ...
                for (tmpk=0; tmpk<8; tmpk++)
                    if (nrtable[tmpi][tmpj][tmpk]>0) {
                        smallestnr=nrtable[tmpi][tmpj][tmpk];
                        leave=1;
                        break;
                    }
            }
