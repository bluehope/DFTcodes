...
int Tran_current_num_step;
double Tran_current_energy_step,Tran_current_cutoff;
double Tran_current_lower_bound,Tran_current_im_energy;
...
char interpolate_filename1[YOUSO10];
char interpolate_filename2[YOUSO10];
double interpolate_c1,interpolate_c2;
    ...
    if (tran_interpolate==1) {
        ...
        if (myid0==0) {
            ...
            printf("  Left lead:  %15.12f   %15.12f   %15.12f (eV)\n",
                   ChemP_e[0]*eV2Hartree,
                   v[1]*eV2Hartree,
                   (interpolate_c1*ChemP_e[0] + interpolate_c2*v[1])*eV2Hartree);
            fflush(stdout);

            printf("  Right lead: %15.12f   %15.12f   %15.12f (eV)\n",
                   ChemP_e[1]*eV2Hartree,
                   v[2]*eV2Hartree,
                   (interpolate_c1*ChemP_e[1] + interpolate_c2*v[2])*eV2Hartree);
            fflush(stdout);
...
int main(int argc, char **argv)
{
    ...
    int myworld1,parallel_mode,num_kloop0;
    int myid0,numprocs0,myid1,numprocs1;
