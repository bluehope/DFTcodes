...

{
    ...
    stat_send1 = malloc(sizeof(MPI_Status)*numprocs1);
    ...
    ****************************************************/
    ...
    for(i=0;i<na_rows;i++){
      for(j=0;j<na_cols;j++){
        ...
        Hs[j*na_rows+i] = H[myworld1][ig][jg];
      }
    }
    */
    ...
    if (g_exx_switch) {
#if !EXX_MIX_DM
        EXX_Debug_Copy_DM(MaxN, CDM, exx, exx_CDM, (SCF_iter<10));
#endif
        ...
        for (spin=0; spin<=SpinP_switch; spin++) {
            EXX_Fock_Cluster(H[spin], &Uexx[spin], exx, exx_CDM[spin], MP);
        }
        if (0==SpinP_switch) {
            Uexx[1] = Uexx[0];
            EXX_Log_Print("EXX: U= %20.12f\n", Uexx[0]);
        } else {
            EXX_Log_Print("EXX: U= %20.12f %20.12f\n", Uexx[0], Uexx[1]);
        }
    }
    /*---------- until here */
...
diagonalize:
    ...
    for (i1=1; i1<=n; i1++) {
        ...
        if(my_prow==prow && my_pcol==pcol) {
            ...
            if(EV_S[i1]<EV_cut0) {
                Hs[(jl-1)*na_rows+il-1] += pow((EV_S[i1]/EV_cut0),-2.0) - 1.0;
            }
            mC_spin_i1 = Hs[(jl-1)*na_rows+il-1];
        }
        else {
            mC_spin_i1 = 0.0;
        }

        MPI_Allreduce(&mC_spin_i1, &C_spin_i1, 1, MPI_DOUBLE, MPI_SUM, MPI_CommWD1[myworld1]);
    ...
    ****************************************************/
    ...
    if(myid1==0) {
        ...
        for(node=0; node<numprocs1; node++) {
            ...
            if(node==0) {
                ...
                MPI_Recv(&irow, 1, MPI_INT, node, 10, MPI_CommWD1[myworld1], stat_send1);
                MPI_Recv(&icol, 1, MPI_INT, node, 20, MPI_CommWD1[myworld1], stat_send1);
                MPI_Recv(&LOCr, 1, MPI_INT, node, 40, MPI_CommWD1[myworld1], stat_send1);
                MPI_Recv(&LOCc, 1, MPI_INT, node, 50, MPI_CommWD1[myworld1], stat_send1);

                MPI_Recv(Cs, LOCr*LOCc, MPI_DOUBLE, node, 30, MPI_CommWD1[myworld1], stat_send1);

                for(i=0; i<LOCr; i++) {
                    for(j=0; j<LOCc; j++) {
                        ig = np_rows*nblk*((i)/nblk) + (i)%nblk + ((np_rows+irow)%np_rows)*nblk + 1;
                        jg = np_cols*nblk*((j)/nblk) + (j)%nblk + ((np_cols+icol)%np_cols)*nblk + 1;
                        C[spin][ig][jg] = Cs[j*LOCr+i];
                    }
                }
            }
        }
        ...
        MPI_Send(&my_prow, 1, MPI_INT, 0, 10, MPI_CommWD1[myworld1]);
        MPI_Send(&my_pcol, 1, MPI_INT, 0, 20, MPI_CommWD1[myworld1]);
        MPI_Send(&na_rows, 1, MPI_INT, 0, 40, MPI_CommWD1[myworld1]);
        MPI_Send(&na_cols, 1, MPI_INT, 0, 50, MPI_CommWD1[myworld1]);
        MPI_Send(Hs, na_rows*na_cols, MPI_DOUBLE, 0, 30, MPI_CommWD1[myworld1]);
    }
    ...
    */
    ...
    for (sp=0; sp<=SpinP_switch; sp++) {
        for(i=1; i<=n; i++) {
            MPI_Bcast(C[sp][i],(n+2),MPI_DOUBLE,Comm_World_StartID1[sp],mpi_comm_level1);
        }
    }
    ...
    ***********************************************/
    ...
    if ( strcasecmp(mode,"scf")==0 ) {
        ...
        ****************************************************/
        ...
        /*---------- added by TOYODA 08/JAN/2010 from here */
        if (g_exx_switch)
        {
            exx_nep  = EXX_Number_of_EP(exx);
            exx_atm1 = EXX_Array_EP_Atom1(exx);
            exx_atm2 = EXX_Array_EP_Atom2(exx);

            /* clear EXX-DM */
            for (spin=0; spin<=SpinP_switch; spin++) {
                for (iep=0; iep<exx_nep; iep++) {
                    GA_AN = exx_atm1[iep]+1;
                    GB_AN = exx_atm2[iep]+1;
                    tnoA  = Spe_Total_CNO[WhatSpecies[GA_AN]];
                    tnoB  = Spe_Total_CNO[WhatSpecies[GB_AN]];
                    for (i=0; i<tnoA; i++) {
                        for (j=0; j<tnoB; j++) {
                            exx_CDM[spin][iep][i][j].r = 0.0;
        ...
        /*---------- added by TOYODA 08/JAN/2010 until here */
        ...
        ****************************************************/
        ...
        Save_DOS_Col(n,MaxN,MP,CntOLP,C,ko);
    }
        ...
        #pragma omp parallel shared(j1,C,S,H,n) private(OMPID,Nthrds,Nprocs,i1,sum_r0,sum_i0,sum_r1,sum_i1,l,l1)
        {
            ...
            for (i1=1+OMPID; i1<=n; i1+=Nthrds) {
                ...
                for (l=1; l<=n; l++) {
                    ...
                    sum_i0 += S[i1][l]*H[j1][l1].i;
                    l1++;

                    sum_r1 += S[i1][l]*H[j1][l1].r;
                    sum_i1 += S[i1][l]*H[j1][l1].i;
                    l1++;
                }
    ...
    ****************************************************/
    ...
    if ( strcasecmp(mode,"scf")==0 ) {
        ...
        ****************************************************/
        ...
        Save_DOS_NonCol(n,MaxN,MP,CntOLP,C,ko);
    }
