        ...
        if (myid == Host_ID) {
            ...
            for (i=0; i<Num_NBO_FCenter; i++) {
                ...
                printf(" #### Global atom num.: %d ( %s ) / NP = %8.4f\n",
                       Gh_AN0,SpeName[Cwan],NP_atom1[Gh_AN0]);
                fflush(stdout);
    ...
    if (myid == Host_ID) {
        if (outputlev==1) {
            ...
            for (FS_AN=0; FS_AN<=FS_atomnum; FS_AN++) {
                ...
                for (L1=0; L1<=Spe_MaxL_Basis[wan1]; L1++) {
                    for (M1=1; M1<=2*L1+1; M1++) {
                        printf(" Posi_alm[%d][%d][%d] = %d \n",FS_AN,L1,M1,Posi_alm[FS_AN][L1][M1]);
                        fflush(stdout);
                    } /* M1 */
                } /* L1 */
            } /* FS_AN */
    ...
    *******************************/
    ...
    for (FS_AN=0; FS_AN<=FS_atomnum; FS_AN++) {
        ...
        for (L1=0; L1<=Spe_MaxL_Basis[wan1]; L1++) {
            ...
            for (i=1; i<=Mmax1; i++) {
                if (W_alm2[i] >= thredens) {
                    M_or_R1[FS_AN][L1][i-1] = 1;
                }
                else                          {
                    M_or_R1[FS_AN][L1][i-1] = 0;
                }
            } /* i */
        ...
        for (FS_AN=0; FS_AN<=FS_atomnum; FS_AN++) {
            ...
            for (L1=0; L1<=Spe_MaxL_Basis[wan1]; L1++) {
                ...
                for (M1=1; M1<=L1*2+1; M1++) {
                    for (i=0; i<leng1; i++) {
                        for (j=0; j<leng1; j++) {
                            printf("%9.5f",S_alm[FS_AN][L1][M1][i][j]);
                            fflush(stdout);
                        }
                        printf("\n");
                        fflush(stdout);
