...
int main(int argc, char *argv[])
{
    ...
    RcvB = (double**)malloc(sizeof(double*)*numprocs);
    for (i=0; i<numprocs; i++) {
        RcvB[i] = (double*)malloc(sizeof(double)*asize1);
    }
    ...
    */
    ...
    for (ID=0; ID<numprocs; ID++) {
        if (ID!=myid) {
            MPI_Recv( &RcvA[ID][0], asize1/2, MPI_DOUBLE, ID, tag, mpi_comm_level1, &stat);
            MPI_Recv( &RcvB[ID][0], asize1/2, MPI_DOUBLE, ID, tag, mpi_comm_level1, &stat);
        }
    }
    MPI_Wait(&request,&stat);
