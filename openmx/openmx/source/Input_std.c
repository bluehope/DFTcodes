...
void Input_std(char *file)
{
    ...
    ****************************************************/
    ...
    /*
    if (fp=input_find("<Mass.of.Atomic.Species")) {
      ...
      if (! input_last("Mass.of.Atomic.Species>")) {

        po++;
        if (myid==Host_ID){
          printf("Format error for Mass.of.Atomic.Species\n");
        }
        MPI_Finalize();
        exit(0);
      }
    ...
    ****************************************************/
    ...
    s_vec[i]="RF";
    i_vec[i]=6;
    i++; /* RF method by hmweng */
    s_vec[i]="DIIS";
    i_vec[i]=7;
    i++;
    s_vec[i]="Constraint_DIIS";
    i_vec[i]=8;
    i++; /* not used */
    s_vec[i]="NVT_NH";
    i_vec[i]=9;
    ...
    s_vec[i]="OptC1";
    i_vec[i]=17;
    i++; /* cell opt with fixed fractional coordinates by SD */
    s_vec[i]="OptC2";
    i_vec[i]=18;
    i++; /* cell opt with fixed fractional coordinates and angles fixed by SD */
    s_vec[i]="OptC3";
    i_vec[i]=19;
    i++; /* cell opt with fixed fractional coordinates, angles fixed and |a1|=|a2|=|a3| by SD */
    s_vec[i]="OptC4";
    i_vec[i]=20;
    i++; /* cell opt with fixed fractional coordinates, angles fixed and |a1|=|a2|!=|a3| by SD */
    s_vec[i]="OptC5";
    i_vec[i]=21;
    i++; /* cell opt with no constraint for cell and coordinates by SD */
    s_vec[i]="RFC1";
    i_vec[i]=22;
    i++; /* cell opt with fixed fractional coordinates by RF */
    s_vec[i]="RFC2";
    i_vec[i]=23;
    i++; /* cell opt with fixed fractional coordinates and angles fixed by RF */
    s_vec[i]="RFC3";
    i_vec[i]=24;
    i++; /* cell opt with fixed fractional coordinates, angles fixed and |a1|=|a2|=|a3| by RF */
    s_vec[i]="RFC4";
    i_vec[i]=25;
    i++; /* cell opt with fixed fractional coordinates, angles fixed and |a1|=|a2|!=|a3| by RF */
    s_vec[i]="RFC5";
    i_vec[i]=26;
    i++; /* cell opt with no constraint for cell and coordinates by RF */
    ...
    ***************************************/
    ...
    TRAN_Input_std(MPI_COMM_WORLD1, Solver, SpinP_switch, filepath, kB,
        ...
        ****************************************************/
        ...
        if (fp=input_find("<Atoms.Unitvectors")) {
            ...
            if (ESM_switch!=0) {
                ...
                if ( tv[1][1]<0.0 || 1.0e-13<fabs(tv[1][2]) || 1.0e-13<fabs(tv[1][3]) ||
                        1.0e-13<fabs(tv[1][1]*tv[1][2]) ||
                        1.0e-13<fabs(tv[1][2]*tv[1][3]) ||
                        1.0e-13<fabs(tv[1][3]*tv[1][1])
                   ) {

                    if (myid==Host_ID) {
                        printf("<ESM:Warning>\n");
                        printf("The a-axis of the unit cell used for the ESM calculation\n");
                        printf("must be (x, 0, 0) in the cartesian coordinate,\n");
                        printf("where x must be POSITIVE.\n\n");
                    }
                ...
                if ( 1.0e-13<fabs(tv[2][1]) || 1.0e-13<fabs(tv[3][1]) ) {
    ...
    *******************************************************/
    ...
    if ( Kerker_factor<0.0 && (Mixing_switch==3 || Mixing_switch==4) ) {
    ...
    ****************************************************/
    ...
    if(dftD_switch) {
        s_vec[1]="Ang";
        s_vec[0]="AU"; /* Ellner CHANGED ORDER */
        i_vec[1]=0;
        i_vec[0]=1;
    ...
    *****************************************************/
    ...
    s_vec[0]="off";
    i_vec[0] = 0;   /* default, off for first MD, later on */
    s_vec[1]="alloff";
    ...
    ****************************************************/
    ...
    if (fp=input_find("<orderN.FNAN+SNAN")) {
        ...
        for (i=1; i<=atomnum; i++) {
            fscanf(fp,"%i %i",&j,&orderN_FNAN_SNAN[i]);
    ...
    if (unfold_electronic_band==1) {
        ...
        if (fp=input_find("<Unfolding.Map")) {
            for (i=0; i<atomnum; i++) {
                ...
                if ((j!=i+1)||(unfold_mapN2n[i]<0)) {
                    printf("Format error in Unfolding.Map! (Values cannot be negative.)\n");
                    po++;
                }
            }
    ...
    ****************************************************/
    ...
    i_vec[0]=Kspace_grid1;
    i_vec[1]=Kspace_grid2, i_vec[2]=Kspace_grid3;
    ...
    ****************************************************/
    ...
    /****************************************************
      parameters for exact exchange methods by TOYODA
    ****************************************************/

    /*---------- added by TOYODA 08/FEB/2010 */
    input_logical("exx.step1_skip", &g_exx_skip1, 0);
    input_logical("exx.step2_skip", &g_exx_skip2, 0);
    input_string("exx.cachedir", g_exx_cachedir, ".");
    input_int("exx.liberi.lmax", &g_exx_liberi_lmax, 20);
    input_int("exx.liberi.ngrid", &g_exx_liberi_ngrid, 1024);
    input_int("exx.liberi.ngl", &g_exx_liberi_ngl, 100);
    input_double("exx.rc_cut", &g_exx_rc_cut, -1.0);
    input_double("exx.w_scr", &g_exx_w_scr, -1.0);
    input_logical("exx.output_DM", &g_exx_switch_output_DM, 0);
    input_logical("exx.rhox", &g_exx_switch_rhox, 0);
    /*--------- until here */
    ...
    ****************************************************/
    ...
    k = 0;
    for (i=1; i<=atomnum; i++) {
        His_Gxyz[0][k] = Gxyz[i][1];
        k++;
        His_Gxyz[0][k] = Gxyz[i][2];
        k++;
        His_Gxyz[0][k] = Gxyz[i][3];
        k++;
    }
                               ...
                               double ***tmp_Wannier_Projector_Hybridize_Matrix)
{
    ...
    *******************************************/
    ...
    /* s */
    strcpy(Name_Wannier_Projectors[  0], "s");
    Num_Projectors[0]  = 1;
    NumL[0][0]=1;
    SelMat[0][0]=0;
    HybridMat[0][0][0]=1.0;
    ...
    /* px */
    strcpy(Name_Wannier_Projectors[  1], "px");
    Num_Projectors[1]  = 1;
    NumL[1][1]=1;
    SelMat[1][0]=0;
    HybridMat[1][0][0]=1.0;
    ...
    /* py */
    strcpy(Name_Wannier_Projectors[  2], "py");
    Num_Projectors[2]  = 1;
    NumL[2][1]=1;
    SelMat[2][0]=1;
    HybridMat[2][0][0]=1.0;
    ...
    /* pz */
    strcpy(Name_Wannier_Projectors[  3], "pz");
    Num_Projectors[3]  = 1;
    NumL[3][1]=1;
    SelMat[3][0]=2;
    HybridMat[3][0][0]=1.0;
    ...
    /* dz2 */
    strcpy(Name_Wannier_Projectors[  4], "dz2");
    Num_Projectors[4]  = 1;
    NumL[4][2]=1;
    SelMat[4][0]=0;
    HybridMat[4][0][0]=1.0;
    ...
    /* dx2-y2 */
    strcpy(Name_Wannier_Projectors[  5], "dx2-y2");
    Num_Projectors[5]  = 1;
    NumL[5][2]=1;
    SelMat[5][0]=1;
    HybridMat[5][0][0]=1.0;
    ...
    /* dxy */
    strcpy(Name_Wannier_Projectors[  6], "dxy");
    Num_Projectors[6]  = 1;
    NumL[6][2]=1;
    SelMat[6][0]=2;
    HybridMat[6][0][0]=1.0;
    ...
    /* dxz */
    strcpy(Name_Wannier_Projectors[  7], "dxz");
    Num_Projectors[7]  = 1;
    NumL[7][2]=1;
    SelMat[7][0]=3;
    HybridMat[7][0][0]=1.0;
    ...
    /* dyz */
    strcpy(Name_Wannier_Projectors[  8], "dyz");
    Num_Projectors[8]  = 1;
    NumL[8][2]=1;
    SelMat[8][0]=4;
    HybridMat[8][0][0]=1.0;
    ...
    /* fz3 */
    strcpy(Name_Wannier_Projectors[  9], "fz3");
    Num_Projectors[9] = 1;
    NumL[9][3]=1;
    SelMat[9][0]=0;
    HybridMat[9][0][0]=1.0;
    ...
    /* fxz2 */
    strcpy(Name_Wannier_Projectors[ 10], "fxz2");
    Num_Projectors[10] = 1;
    NumL[10][3]=1;
    SelMat[10][0]=1;
    ...
    /* fyz2 */
    strcpy(Name_Wannier_Projectors[ 11], "fyz2");
    Num_Projectors[11] = 1;
    NumL[11][3]=1;
    SelMat[11][0]=2;
    ...
    /* fzx2 */
    strcpy(Name_Wannier_Projectors[ 12], "fzx2");
    Num_Projectors[12] = 1;
    NumL[12][3]=1;
    SelMat[12][0]=3;
    ...
    /* fxyz */
    strcpy(Name_Wannier_Projectors[ 13], "fxyz");
    Num_Projectors[13] = 1;
    NumL[13][3]=1;
    SelMat[13][0]=4;
    ...
    /* fx3-3xy2 */
    strcpy(Name_Wannier_Projectors[ 14], "fx3-3xy2");
    Num_Projectors[14] = 1;
    NumL[14][3]=1;
    SelMat[14][0]=5;
    ...
    /* f3yx2-y3 */
    strcpy(Name_Wannier_Projectors[ 15], "f3yx2-y3");
    Num_Projectors[15] = 1;
    NumL[15][3]=1;
    SelMat[15][0]=6;
    ...
    /* sp */
    strcpy(Name_Wannier_Projectors[ 16], "sp");
    Num_Projectors[16] = 2;
    NumL[16][0]=1;
    NumL[16][1]=1;
    SelMat[16][0]=0;  /* this sp means the s and px hybirdization, therefore s and px should be selected. */
    SelMat[16][1]=1;
    HybridMat[16][0][0]=1.0/sqrt(2.0);
    HybridMat[16][0][1]=1.0/sqrt(2.0);
    HybridMat[16][1][0]=1.0/sqrt(2.0);
    HybridMat[16][1][1]=-1.0/sqrt(2.0);

    /* sp2 */
    strcpy(Name_Wannier_Projectors[ 17], "sp2");
    Num_Projectors[17] = 3;
    NumL[17][0]=1;
    NumL[17][1]=1;
    ...
    /* sp3 */
    strcpy(Name_Wannier_Projectors[ 18], "sp3");
    Num_Projectors[18] = 4;
    NumL[18][0]=1;
    NumL[18][1]=1;
    ...
    HybridMat[21][0][0]=1.0;
    HybridMat[21][0][1]=0.0;
    HybridMat[21][0][2]=0.0;
    HybridMat[21][1][0]=0.0;
    HybridMat[21][1][1]=1.0;
    HybridMat[21][1][2]=0.0;
    HybridMat[21][2][0]=0.0;
    HybridMat[21][2][1]=0.0;
    HybridMat[21][2][2]=1.0;
