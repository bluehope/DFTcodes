...
void Stress3()
{
    ...
    **********************************************************/
    ...
    {
        int i, j;
        My_Stress_threads=(double**)malloc(sizeof(double*)*(gNthrds));
        for (i=0; i<gNthrds; i++) {
            My_Stress_threads[i] = (double*)malloc(sizeof(double)*9);
            for (j=0; j<9; j++) {
                My_Stress_threads[i][j] = 0.0;
            }
        }
    }
    /* end(yshiihara) */
    ...
    #pragma omp parallel
    {
        ...
        /* Several commented lines in Force.c are skipped here...*/
        ...
        for (Mc_AN = 1; Mc_AN <= Matomnum; Mc_AN++) {
            ...
            ***********************************/
            ...
            /* begin(yshiihara) */
            double sumt[9],sumt1[9];
            {
                int i;
                for(i=0; i<9; i++) sumt[i]    = 0.0;
                for(i=0; i<9; i++) sumt1[i]    = 0.0;
            }
            /* end(yshiihara) */
            ...
            if (SpinP_switch==0 || SpinP_switch==1) {
                ...
                for (spin=0; spin<=SpinP_switch; spin++) {
                    ...
                    for (Nc=0; Nc<GridN_Atom[Gc_AN]; Nc++) {
                        ...
                        sumt[0] -= dxyz[Nc][0]*dDen_Grid[Nc][spin][0]*Vpt;
                        sumt[1] -= dxyz[Nc][1]*dDen_Grid[Nc][spin][0]*Vpt;
                        sumt[2] -= dxyz[Nc][2]*dDen_Grid[Nc][spin][0]*Vpt;
                        sumt[3] -= dxyz[Nc][0]*dDen_Grid[Nc][spin][1]*Vpt;
                        sumt[4] -= dxyz[Nc][1]*dDen_Grid[Nc][spin][1]*Vpt;
                        sumt[5] -= dxyz[Nc][2]*dDen_Grid[Nc][spin][1]*Vpt;
                        sumt[6] -= dxyz[Nc][0]*dDen_Grid[Nc][spin][2]*Vpt;
                        sumt[7] -= dxyz[Nc][1]*dDen_Grid[Nc][spin][2]*Vpt;
                        sumt[8] -= dxyz[Nc][2]*dDen_Grid[Nc][spin][2]*Vpt;
                        /* end(yshiihara) */
                    }
                }
                ...
                /* spin non-collinear calculation is not currently supported in Stress.c */
            ...
            /* begin(yshiihara) */
            My_Stress_threads[OMPID][0] += sumt[0]*GridVol;
            My_Stress_threads[OMPID][1] += sumt[1]*GridVol;
            My_Stress_threads[OMPID][2] += sumt[2]*GridVol;
            My_Stress_threads[OMPID][3] += sumt[3]*GridVol;
            My_Stress_threads[OMPID][4] += sumt[4]*GridVol;
            My_Stress_threads[OMPID][5] += sumt[5]*GridVol;
            My_Stress_threads[OMPID][6] += sumt[6]*GridVol;
            My_Stress_threads[OMPID][7] += sumt[7]*GridVol;
            My_Stress_threads[OMPID][8] += sumt[8]*GridVol;
            /* end(yshiihara)   */
    ...
    **********************************************************/
    ...
    if (XC_switch==4) {
        ...
        Set_XC_Grid(XC_P_switch_stress,XC_switch,
