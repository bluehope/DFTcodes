...
void EXX_on_OpenMX_Init(MPI_Comm comm)
{
    ...
    /* user values */
    if (g_exx_rc_cut > 1e-10) {
        rc_cut = g_exx_rc_cut;
    }
    if (g_exx_w_scr > 1e-10) {
        w_scr = g_exx_w_scr;
    }
    ...
    /* species */
    for (i=0; i<natom; i++) {
        atom_sp[i] = WhatSpecies[i+1];
    }

    /* cut-off radii */
    for (i=0; i<nspec; i++) {
        spec_rc[i] = Spe_Atom_Cut1[i];
    }
    ...
    if (Host_ID==myrank) {
        printf("<EXX_Init> EXX Initialized\n");
    }
...
)
{
    ...
    for (i=0; i<nep; i++) {
        if (ia1==ep_atom1[i] && ia2==ep_atom2[i] && ic==ep_cell[i]) {
            break;
        }
    }
...
)
{
    ...
    for (i=0; i<nep; i++) {
        if (ia1==ep_atom1[i] && ia2==ep_atom2[i]) {
            break;
        }
    }
...
)
{
    ...
    H_n = 0;
    for (i=1; i<=atomnum; i++) {
        H_n += Spe_Total_CNO[WhatSpecies[i]];
    }
