...

{
    ...
    /* calculate group velocity and determine left and right going modes*/
    {
        ...
        for ( i=0; i<m; i++ ) {
            ...
            if ( mode == 0 ) {
                ...
                for ( j=0; j<n; j++) {
                    test1[j].r = VR[i*m+j].r;
                    test1[j].i = VR[i*m+j].i;
                    test2[j].r = test1[j].r;
                    test2[j].i = -test1[j].i;
                }
                a.r = 1.0;
                ...
                temp2i = t2r*r2i + t2i*r2r;
                temp2r = t2r*r2r - t2i*r2i;
                temp3i = t3r*r3i + t3i*r3r;
                temp3r = t3r*r3r - t3i*r3i;
        ...
        if ( ns1 != n || ns2 != n || ns1+ns2 != m) {
            printf("Determination of left and right going modes are problematic...\n");
        }
        ...
        for ( i=0; i<m; i++) {
            if ( move[i] == 1 ) {
                ...
                for ( j=0; j<n; j++) {
                    V1[ns1*n+j].r   = VR[i*m+j].r;
                    V1[ns1*n+j].i  = VR[i*m+j].i;
                    V1t[ns1*n+j].r  = VR[i*m+j].r;
                    V1t[ns1*n+j].i = VR[i*m+j].i;
                }
                ns1=ns1+1;
            } else if ( move[i] == -1 ) {
                ...
                for ( j=0; j<n; j++) {
                    V2[ns2*n+j].r  = VR[i*m+j].r;
                    V2[ns2*n+j].i  = VR[i*m+j].i;
                    V2t[ns2*n+j].r = VR[i*m+j].r;
                    V2t[ns2*n+j].i = VR[i*m+j].i;
                }
                ns2=ns2+1;
            }
        } /* for ( i=0; i<m; i++) */
    ...
    /* calculate the inverse of V */
    {
        ...
        for ( i=0; i<n; i++) {
            ...
            for ( j=0; j<n; j++) {
                test1[j].r = V1[i*n+j].r;
                test1[j].i = V1[i*n+j].i;
                test2[j].r = V1t[j*n+i].r;
                test2[j].i = V1t[j*n+i].i;
            }
            ...
            for ( j=0; j<n; j++) {
                test1[j].r = V2[i*n+j].r;
                test1[j].i = V2[i*n+j].i;
                test2[j].r = V2t[j*n+i].r;
                test2[j].i = V2t[j*n+i].i;
            }
